...
| Version provenance | Inherit a workspace version; derive `TOOL_VERSION` from `CARGO_PKG_VERSION`; use release-please Cargo-workspace/linked versions. | Repeated constants or tag-only versioning. | One value drives binary, release metadata, npm pins, archive names, OCI labels, and Cargo versions. |
