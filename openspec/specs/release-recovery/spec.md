...
This capability was introduced to recover `v0.3.0` in `yacosta738/codegauge`, which remained
untagged because global and root Release Please configuration had suppressed GitHub Release
creation. The authoritative historical identity for that recovery is:
...
- historical `.release-please-manifest.json` had **13** entries; the current manifest has **14**
  entries, the delta being the addition of `crates/codegauge-provider-typescript`

The capability is durable: it defines the contract for any future merged release that needs
explicit recovery, not only `v0.3.0`. The archived change folder
`openspec/changes/archive/2026-08-30-release-recovery-idempotent-ownership/` preserves the
proposal, design, tasks, apply progress, verify report, QA report, and the original delta spec
as the full audit trail.

Live recovery is never authorized by this spec. Live recovery still requires
`vars.RELEASE_RECOVERY_LIVE_ENABLED=true`, the protected `release-recovery-live` environment
approval, and an explicit operator invocation.
