use codegauge_model::{
    ...
    InputArtifact, JAVA_JACOCO_V1, ProfileId, Provenance, ResultDocument, ResultSchemaId, Summary,
    SymbolIdentity, SymbolResult, ToolInfo,
};
...
fn ids_identity_and_measurements_are_stable() {
    ...
    assert_eq!(
        (first.language.as_str(), first.kind.as_str()),
        ("java", "method")
    );
