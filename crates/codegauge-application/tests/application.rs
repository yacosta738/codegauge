...
use codegauge_model::{AnalysisStatus,ComplexityMeasurement,CoverageMeasurement,DerivedMetrics,ErrorCode,ProfileId,SymbolIdentity,SymbolResult};
...
#[test] fn reader_hashes_exact_bytes_and_normalizes_only_display_path() {
 ...
 assert_eq!((a.path,a.sha256),("../../fixtures/jacoco/valid-methods.xml".into(),"6deddb631c24f78d5fe7eff07009a4d7c6714c2317689139bd4111e0cfd0bf2e".into()));
}
...
#[test] fn analyzer_uses_core_sorts_summarizes_and_sets_status() {
 ...
 assert_eq!(r.analysis.status,AnalysisStatus::Complete); assert_eq!(r.symbols.iter().map(|s|s.symbol.id.as_str()).collect::<Vec<_>>(),["java:B","java:a"]);
...
fn sym(id:&str,cc:f64,ratio:f64)->SymbolResult{SymbolResult{symbol:SymbolIdentity{id:id.into(),language:"java".into(),kind:"method".into(),class_vm:"C".into(),name:"m".into(),descriptor:"()V".into()},complexity:Some(ComplexityMeasurement{value:cc,metric:"cyclomatic".into(),semantics:"jacoco-cyclomatic".into(),provider:"jacoco".into()}),coverage:Some(CoverageMeasurement{ratio,covered:1,missed:0,metric:"instruction".into(),semantics:"jacoco-instruction".into(),provider:"jacoco".into()}),metrics:DerivedMetrics{crap:Some(-1.0)}}}
